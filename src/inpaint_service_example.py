...
def start(origin_img_path, input_img_path, mask_img_path, id, time):
    ...
    for target in palettes:
        input_image = {
            "image": Image.open(input_img_path%target).resize((1024, 512), Image.LANCZOS),
            "mask": Image.open(mask_img_path%target).resize((1024, 512), Image.LANCZOS)
        }
        masks.append(np.array(input_image["mask"]))
        prompt = "masterpiece, best quality, realistic, %s, photo, cityscapes"%target
        neg_prompt = "watermark, username, error, anime, nsfw, low quality, lowres, animal, painting, poster"
        ddim_steps = 50
        num_samples = 1
        scale = 9.0
        seed = 11451419
...
NAMESPACE_ANONYMIZE = uuid.uuid3(uuid.NAMESPACE_DNS, "anonymize")
...
def main():
    ...
    origin_img_path = request.form["origin"]
    input_img_path = request.form["input"]
    mask_img_path = request.form["mask"]
    path_uuid = uuid.uuid3(uuid.NAMESPACE_DNS, origin_img_path).hex + uuid.uuid3(uuid.NAMESPACE_DNS, input_img_path).hex + uuid.uuid3(uuid.NAMESPACE_DNS, mask_img_path).hex
    res = { "status": 1200, "id": str(uuid.uuid3(NAMESPACE_ANONYMIZE, path_uuid)), "time": int(time.time()) }
    is_running = True
    t = threading.Thread(target = start, args = (origin_img_path, input_img_path, mask_img_path, res["id"], res["time"]))
    t.setDaemon(True)
    t.start()
...
def status():
    return json.dumps({"status": 1200, "message": 'operational'})
...
def ui():
    return render_template("inpaint-ui.html")

if __name__ == "__main__":
    app.run(port=2022, host="127.0.0.1", debug=False, use_reloader=False)